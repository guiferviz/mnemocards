...
class AutoGenerateTsvUnitTestSuit(unittest.TestCase):
    ...
    example_tsv_filename = "/home/otterman/python_projects/active/mnemocards/tests/test_tsv_config.tsv"
    ...
    def test_tsv_are_saved(self):
        ...
        test_tsv_config = open("tests/test_tsv_config.tsv", 'r')
        output_dir = "tests/test_card_folder"
        language_pair = "en_ru"
        ...
        result_tsv = open("tests/test_card_folder/en_ru.tsv", 'r')

        self.assertEqual(test_tsv_config.read(), result_tsv.read())

        test_tsv_config.close()
        result_tsv.close()
        os.remove('tests/test_card_folder/en_ru.tsv')
